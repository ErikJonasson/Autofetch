...
import org.hibernate.cfg.HbmBinder.CollectionSecondPass;
import org.hibernate.cfg.HbmBinder.IdentifierCollectionSecondPass;
import org.hibernate.cfg.HbmBinder.ListSecondPass;
import org.hibernate.cfg.HbmBinder.MapSecondPass;
...
public final class AutofetchHbmBinder {
	...
	private static final CoreMessageLogger LOG = Logger.getMessageLogger(CoreMessageLogger.class,
			AutofetchHbmBinder.class.getName());
	...
	/**
	 * The main contract into the hbm.xml-based binder. Performs necessary binding
	 * operations represented by the given DOM.
	 * 
	 * @param doc
		...
		if (definitionNode != null) {
			try {
				auxDbObject = (AuxiliaryDatabaseObject) ReflectHelper
						.classForName(definitionNode.attributeValue("class")).newInstance();
			} catch (ClassNotFoundException e) {
				throw new MappingException("could not locate custom database object class ["
						+ definitionNode.attributeValue("class") + "]");
			} catch (Throwable t) {
				throw new MappingException("could not instantiate custom database object class ["
						+ definitionNode.attributeValue("class") + "]");
			}
		} else {
			auxDbObject = new SimpleAuxiliaryDatabaseObject(auxDbObjectNode.elementTextTrim("create"),
	...
	private static void bindRootPersistentClassCommonValues(Element node, java.util.Map inheritedMetas,
			Mappings mappings, RootClass entity) throws MappingException {
	...
	private static void bindSimpleId(Element idNode, RootClass entity, Mappings mappings, java.util.Map inheritedMetas)
		 ...
		 * MappingException( "illegal use of an array as an identifier (arrays don't
		 * reimplement equals)" );
		 */
	...
	private static void bindVersioningProperty(Table table, Element subnode, Mappings mappings, String name,
			RootClass entity, java.util.Map inheritedMetas) {
	...
	private static void bindPojoRepresentation(Element node, PersistentClass entity, Mappings mappings,
		...
		entity.addTuplizer(EntityMode.POJO, "org.autofetch.hibernate.AutofetchTuplizer");
	}

	private static void bindDom4jRepresentation(Element node, PersistentClass entity, Mappings mappings,
	...
	private static void bindMapRepresentation(Element node, PersistentClass entity, Mappings mappings,
			java.util.Map inheritedMetas) {
	...
	private static void bindPersistentClassCommonValues(Element node, PersistentClass entity, Mappings mappings,
			java.util.Map inheritedMetas) throws MappingException {
		// DISCRIMINATOR
		Attribute discriminatorNode = node.attribute("discriminator-value");
		entity.setDiscriminatorValue(
				(discriminatorNode == null) ? entity.getEntityName() : discriminatorNode.getValue());
		...
		entity.setOptimisticLockStyle(getOptimisticLockMode(olNode));
		...
		Boolean isAbstract = abstractNode == null ? null
				: "true".equals(abstractNode.getValue()) ? Boolean.TRUE
						: "false".equals(abstractNode.getValue()) ? Boolean.FALSE : null;
	...
	private static ExecuteUpdateResultCheckStyle getResultCheckStyle(Element element, boolean callable)
			throws MappingException {
	...
	public static void bindUnionSubclass(Element node, UnionSubclass unionSubclass, Mappings mappings,
			java.util.Map inheritedMetas) throws MappingException {
		...
		Table mytable = mappings.addDenormalizedTable(schema, catalog,
				...
				unionSubclass.isAbstract() != null && unionSubclass.isAbstract().booleanValue(), getSubselect(node),
	...
	private static String getClassTableName(PersistentClass model, Element node, String schema, String catalog,
	...
	public static void bindJoinedSubclass(Element node, JoinedSubclass joinedSubclass, Mappings mappings,
			java.util.Map inheritedMetas) throws MappingException {
		...
		Table mytable = mappings.addTable(schema, catalog,
				getClassTableName(joinedSubclass, node, schema, catalog, null, mappings), getSubselect(node), false);
	...
	private static void bindJoin(Element node, Join join, Mappings mappings, java.util.Map inheritedMetas)
		...
		Table table = mappings.addTable(schema, catalog,
				getClassTableName(persistentClass, node, schema, catalog, primaryTable, mappings), getSubselect(node),
				false);
		join.setTable(table);
		bindComment(table, node);
		...
		while (iter.hasNext()) {
			...
			if ("many-to-one".equals(name)) {
				...
				bindComponent(subnode, (Component) value, join.getPersistentClass().getClassName(), propertyName,
						subpath, true, false, mappings, inheritedMetas, false);
			}

			if (value != null) {
				Property prop = createProperty(value, propertyName, persistentClass.getEntityName(), subnode, mappings,
						inheritedMetas);
	...
	public static void bindColumns(final Element node, final SimpleValue simpleValue, final boolean isNullable,
			final boolean autoColumn, final String propertyPath, final Mappings mappings) throws MappingException {
		...
		if (columnAttribute == null) {
			...
			String logicalColumnName = mappings.getNamingStrategy().logicalColumnName(columnAttribute.getValue(),
					propertyPath);
		...
		if (autoColumn && simpleValue.getColumnSpan() == 0) {
			 ...
			 * or at a slightly higer level in the stack (to get all the information we
			 * need) Right now AutofetchHbmBinder does not support the
			 */
	...
	// XML
	public static void bindSimpleValue(Element node, SimpleValue simpleValue, boolean isNullable, String path,
		...
		if (generationTiming == GenerationTiming.ALWAYS || generationTiming == GenerationTiming.INSERT) {
			...
			if (property.isInsertable()) {
				if (insertNode == null) {
					...
					throw new MappingException("cannot specify both insert=\"true\" and generated=\""
							+ generationTiming.name() + "\" for property: " + propName);
				}
			}
			...
			if (property.isUpdateable() && generationTiming == GenerationTiming.ALWAYS) {
				if (updateNode == null) {
					...
					// which constitutes an illegal combo
					throw new MappingException("cannot specify both update=\"true\" and generated=\""
							+ generationTiming.name() + "\" for property: " + propName);
				}
			}
		}

		boolean isLazyable = "property".equals(node.getName()) || "component".equals(node.getName())
				|| "many-to-one".equals(node.getName()) || "one-to-one".equals(node.getName())
	 ...
	 */
	public static void bindCollection(Element node, Collection collection, String className, String path,
		...
		if (oneToManyNode != null) {
			...
			if (tableNode != null) {
				...
				// FIXME we don't have the associated entity table name here, has to be done in
				// a second pass
				if (node.element("element") != null || node.element("composite-element") != null) {
					tableName = getNamingStrategyDelegate(mappings).determineImplicitElementCollectionTableName(
							collection.getOwner().getClassName(), collection.getOwner().getJpaEntityName(),
							logicalOwnerTableName, path);
				} else {
					tableName = getNamingStrategyDelegate(mappings).determineImplicitEntityAssociationJoinTableName(
							collection.getOwner().getClassName(), collection.getOwner().getJpaEntityName(),
							logicalOwnerTableName, null, null, null, path);
				}
			...
			if (LOG.isDebugEnabled()) {
				LOG.debugf("Mapping collection: %s -> %s", collection.getRole(),
	...
	// if (tableNode != null) {
	// tableName =
	// getNamingStrategyDelegate(mappings).toPhysicalTableName(tableNode.getValue());
	// } else {
	...
	// if ( node.element( "element" ) != null || node.element( "composite-element" )
	// != null ) {
	// tableName = getNamingStrategyDelegate( mappings
	...
	private static void initLaziness(Element node, Fetchable fetchable, Mappings mappings, String proxyVal,
			boolean defaultLazy) {
		Attribute lazyNode = node.attribute("lazy");
		boolean isLazyTrue = lazyNode == null ? defaultLazy && fetchable.isLazy() : // fetch="join" overrides default
		// laziness
	...
	private static void bindColumnsOrFormula(Element node, SimpleValue simpleValue, String path, boolean isNullable,
			Mappings mappings) {
	...
	public static void bindManyToOne(Element node, ManyToOne manyToOne, String path, boolean isNullable,
			Mappings mappings) throws MappingException {
	...
	public static void bindComposite(Element node, Component component, String path, boolean isNullable,
			Mappings mappings, java.util.Map inheritedMetas) throws MappingException {
		bindComponent(node, component, null, null, path, isNullable, false, mappings, inheritedMetas, false);
	}

	public static void bindCompositeId(Element node, Component component, PersistentClass persistentClass,
			String propertyName, Mappings mappings, java.util.Map inheritedMetas) throws MappingException {

		component.setKey(true);

		String path = StringHelper.qualify(persistentClass.getEntityName(), propertyName == null ? "id" : propertyName);

		bindComponent(node, component, persistentClass.getClassName(), propertyName, path, false,
				node.attribute("class") == null && propertyName == null, mappings, inheritedMetas, false);
		...
		if ("true".equals(node.attributeValue("mapped"))) {
			...
			bindComponent(node, mapper, persistentClass.getClassName(), null, path, false, true, mappings,
					inheritedMetas, true);
	...
	public static void bindComponent(Element node, Component component, String ownerClassName, String parentProperty,
			String path, boolean isNullable, boolean isEmbedded, Mappings mappings, java.util.Map inheritedMetas,
		...
		while (iter.hasNext()) {
			...
			if (collectType != null) {
				...
				value = new Component(mappings, component); // a nested composite
				// element
				bindComponent(subnode, (Component) value, component.getComponentClassName(), propertyName, subpath,
						isNullable, isEmbedded, mappings, inheritedMetas, isIdentifierMapper);
			} else if ("parent".equals(name)) {
				component.setParentProperty(propertyName);
			}

			if (value != null) {
				Property property = createProperty(value, propertyName, component.getComponentClassName(), subnode,
	...
	protected static void createClassProperties(Element node, PersistentClass persistentClass, Mappings mappings,
			java.util.Map inheritedMetas) throws MappingException {
		createClassProperties(node, persistentClass, mappings, inheritedMetas, null, true, true, false);
	}

	protected static void createClassProperties(Element node, PersistentClass persistentClass, Mappings mappings,
			java.util.Map inheritedMetas, UniqueKey uniqueKey, boolean mutable, boolean nullable, boolean naturalId)
			throws MappingException {
		...
		while (iter.hasNext()) {
			...
			Value value = null;
			if (collectType != null) {
				Collection collection = collectType.create(subnode, StringHelper.qualify(entityName, propertyName),
				...
				bindComponent(subnode, (Component) value, persistentClass.getClassName(), propertyName, subpath, true,
						"properties".equals(name), mappings, inheritedMetas, false);
			...
			if (value != null) {
				Property property = createProperty(value, propertyName, persistentClass.getClassName(), subnode,
						mappings, inheritedMetas);
				if (!mutable)
					property.setUpdateable(false);
				if (naturalId)
	...
	private static Property createProperty(final Value value, final String propertyName, final String className,
			final Element subnode, final Mappings mappings, java.util.Map inheritedMetas) throws MappingException {

		if (StringHelper.isEmpty(propertyName)) {
			throw new MappingException(
					subnode.getName() + " mapping must defined a name attribute [" + className + "]");
		}
	...
	private static void handleUnionSubclass(PersistentClass model, Mappings mappings, Element subnode,
			java.util.Map inheritedMetas) throws MappingException {
	 ...
	 */
	public static void bindListSecondPass(Element node, org.hibernate.mapping.List list, java.util.Map classes,
			Mappings mappings, java.util.Map inheritedMetas) throws MappingException {
	...
	public static void bindIdentifierCollectionSecondPass(Element node, IdentifierCollection collection,
			java.util.Map persistentClasses, Mappings mappings, java.util.Map inheritedMetas) throws MappingException {
	 ...
	 * Called for Maps
	 */
	public static void bindMapSecondPass(Element node, Map map, java.util.Map classes, Mappings mappings,
			java.util.Map inheritedMetas) throws MappingException {
		...
		while (iter.hasNext()) {
			...
			if ("index".equals(name) || "map-key".equals(name)) {
				...
				bindSimpleValue(subnode, value, map.isOneToMany(), IndexedCollection.DEFAULT_INDEX_COLUMN_NAME,
						mappings);
				...
				bindComposite(subnode, component, map.getRole() + ".index", map.isOneToMany(), mappings,
						inheritedMetas);
	 ...
	 * Called for all collections
	 */
	public static void bindCollectionSecondPass(Element node, Collection collection, java.util.Map persistentClasses,
			Mappings mappings, java.util.Map inheritedMetas) throws MappingException {
	...
	private static void bindManyToManySubelements(Collection collection, Element manyToManyNode, Mappings model)
			throws MappingException {
		// Bind the where
		...
		if ((filters.hasNext() || whereCondition != null) && collection.getFetchMode() == FetchMode.JOIN
				&& collection.getElement().getFetchMode() != FetchMode.JOIN) {
			throw new MappingException("many-to-many defining filter or where without join fetching "
		...
		while (filters.hasNext()) {
			...
			if (debugEnabled) {
				LOG.debugf("Applying many-to-many filter [%s] as [%s] to role [%s]", name, condition,
						collection.getRole());
			}
			String autoAliasInjectionText = filterElement.attributeValue("autoAliasInjection");
			boolean autoAliasInjection = StringHelper.isEmpty(autoAliasInjectionText) ? true
					: Boolean.parseBoolean(autoAliasInjectionText);
	...
	private static void bindNamedQuery(Element queryElem, String path, Mappings mappings) {
		...
		NamedQueryDefinition namedQuery = new NamedQueryDefinitionBuilder().setName(queryName).setQuery(query)
				.setCacheable(cacheable).setCacheRegion(region).setTimeout(timeout).setFetchSize(fetchSize)
				.setFlushMode(FlushMode.interpretExternalSetting(queryElem.attributeValue("flush-mode")))
				.setCacheMode(CacheMode.interpretExternalSetting(cacheMode)).setReadOnly(readOnly).setComment(comment)
				.setParameterTypes(getParameterTypes(queryElem)).createNamedQueryDefinition();
	...
	private static void bindNamedSQLQuery(Element queryElem, String path, Mappings mappings) {
		mappings.addSecondPass(new AutofetchNamedSQLQuerySecondPass(queryElem, path, mappings));
	}
	...
	static class CollectionSecondPass extends org.hibernate.cfg.CollectionSecondPass {
		...
		public void secondPass(java.util.Map persistentClasses, java.util.Map inheritedMetas) throws MappingException {
			AutofetchHbmBinder.bindCollectionSecondPass(node, collection, persistentClasses, mappings, inheritedMetas);
		}
	}
	...
	static class IdentifierCollectionSecondPass extends CollectionSecondPass {
		...
		public void secondPass(java.util.Map persistentClasses, java.util.Map inheritedMetas) throws MappingException {
			AutofetchHbmBinder.bindIdentifierCollectionSecondPass(node, (IdentifierCollection) collection,
	...
	static class MapSecondPass extends CollectionSecondPass {
		...
		public void secondPass(java.util.Map persistentClasses, java.util.Map inheritedMetas) throws MappingException {
			AutofetchHbmBinder.bindMapSecondPass(node, (Map) collection, persistentClasses, mappings, inheritedMetas);
		}
	...
	static class ListSecondPass extends CollectionSecondPass {
		...
		public void secondPass(java.util.Map persistentClasses, java.util.Map inheritedMetas) throws MappingException {
			AutofetchHbmBinder.bindListSecondPass(node, (org.hibernate.mapping.List) collection, persistentClasses,
					mappings, inheritedMetas);
		}
	...
	/**
	 * This class replaces org.hibernate.mapping.Set. The main change is to return
	 * our own custom type for sets.
	 * 
	...
	private static class Set extends org.hibernate.mapping.Set {
		...
		public org.hibernate.type.CollectionType getDefaultCollectionType() {
			if (isSorted()) {
				return getMappings().getTypeResolver().getTypeFactory().sortedSet(getRole(),
	...
	/**
	 * This class replaces org.hibernate.mapping.List. The main change is to return
	 * our own custom type for List.
	 * 
	 * @author Ali Ibrahim <aibrahim@cs.utexas.edu>
	 *
	 */
	...
	abstract static class CollectionType {
		...
		public abstract Collection create(Element node, String path, PersistentClass owner, Mappings mappings,
				java.util.Map inheritedMetas) throws MappingException;
		...
		private static final CollectionType MAP = new CollectionType("map") {
			public Collection create(Element node, String path, PersistentClass owner, Mappings mappings,
					java.util.Map inheritedMetas) throws MappingException {
		...
		private static final CollectionType SET = new CollectionType("set") {
			public Collection create(Element node, String path, PersistentClass owner, Mappings mappings,
					java.util.Map inheritedMetas) throws MappingException {
				AutofetchHbmBinder.Set set = new AutofetchHbmBinder.Set(mappings, owner);
		...
		private static final CollectionType LIST = new CollectionType("list") {
			public Collection create(Element node, String path, PersistentClass owner, Mappings mappings,
					java.util.Map inheritedMetas) throws MappingException {
				org.hibernate.mapping.List list = new List(mappings, owner);
		...
		private static final CollectionType BAG = new CollectionType("bag") {
			public Collection create(Element node, String path, PersistentClass owner, Mappings mappings,
					java.util.Map inheritedMetas) throws MappingException {
				AutofetchHbmBinder.Bag bag = new AutofetchHbmBinder.Bag(mappings, owner);
		...
		private static final CollectionType IDBAG = new CollectionType("idbag") {
			public Collection create(Element node, String path, PersistentClass owner, Mappings mappings,
					java.util.Map inheritedMetas) throws MappingException {
				AutofetchHbmBinder.IdentifierBag bag = new AutofetchHbmBinder.IdentifierBag(mappings, owner);
				bindCollection(node, bag, owner.getEntityName(), path, mappings, inheritedMetas);
				return bag;
			}
		};
	...
	private static OptimisticLockStyle getOptimisticLockMode(Attribute olAtt) throws MappingException {
		if (olAtt == null) {
	...
	/**
	 * For the given document, locate all extends attributes which refer to entities
	 * (entity-name or class-name) not defined within said document.
	 *
	 * @param doc
	...
	/**
	 * Given an entity-containing-element (startNode) recursively locate all entity
	 * names defined within that element.
	 *
	 * @param mappings
	 *            The already processed mappings
	 * @param startNode
	 ...
	 * @param handler
	 *            The thing that knows what to do whenever we recognize an
	 *            entity-name
	 */
	private static void recognizeEntities(Mappings mappings, final Element startNode, EntityElementHandler handler) {
